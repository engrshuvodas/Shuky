...
def resource_path(relative_path):
    ...
    - In dev mode (running from src/): resolves relative to the project root.
    - In PyInstaller EXE mode: resolves relative to sys._MEIPASS (the temp bundle dir).
    """
    try:
        # PyInstaller extracts files to a temp folder at runtime
        base_path = sys._MEIPASS
    except AttributeError:
        # Running from source: go one level up from src/ to project root
...
DEFAULT_PARAMS = {
    'char_delay_min': 0.665,    # UPDATED default min
    'char_delay_max': 1.258,    # UPDATED default max
...
def reset_to_defaults():
    # Clear text entries in timing controls and set defaults
    for widget in timing_controls.frame.winfo_children():
        if isinstance(widget, tk.Entry):
            widget.delete(0, tk.END)

    # Set requested defaults (non-random for char delay as you specified)
    timing_controls.char_min.insert(0, DEFAULT_PARAMS['char_delay_min'])
    timing_controls.char_max.insert(0, DEFAULT_PARAMS['char_delay_max'])
...
def start_typing():
    ...
    threading.Thread(target=type_text, args=(text, params), daemon=True).start()
    start_timer()
...
def type_text(text, params):
    ...
    Main typing engine.
    Behavior:
      - Visible human-like mistakes can occur (mistake_trigger).
      - If params['error_rate'] == 0 -> all mistakes will be corrected (backspace + correct char).
      - If params['error_rate'] > 0 -> some mistakes will be left uncorrected according to error_rate %.
    """
    global stop_flag, pause_flag, typing_active
    ...
    # small safety: ensure char_delay min < max
    if char_delay[0] > char_delay[1]:
        char_delay = (char_delay[1], char_delay[0])
    ...
    for line in lines:
        ...
        for i, char in enumerate(line):
            ...
            # --- Decide whether to show a human-like mistake visually ---
            # We show a "visible mistake" occasionally to look human.
            # That does NOT mean final output will keep that mistake when error_rate == 0.
            mistake_trigger = False
            if char not in [' ', '\t', '\n'] and params['auto_correct']:
                # small chance to create a visible wrong char (human-like)
                # Adjust probability if you want more/less visible mistakes (10% currently)
                if random.randint(1, 100) <= 10:
                    mistake_trigger = True

            # Determine whether the mistake should remain in final output
            # Only possible if the configured error_rate > 0
            keep_mistake = False
            if params['error_rate'] > 0:
                # if a visible mistake happened, decide whether to keep it based on error_rate %
                if random.randint(1, 100) <= params['error_rate']:
                    keep_mistake = True

            if mistake_trigger:
                # Create a plausible wrong character (simple offset)
                wrong_char = chr(max(32, ord(char) + random.randint(-2, 2)))
                # Type wrong char to appear human
                pyautogui.write(wrong_char)
                time.sleep(random.uniform(*error_correction))

                if keep_mistake:
                    # If we decide to keep the mistake (error_rate > 0 and selected),
                    # do NOT press backspace — leave the wrong char in output.
                    # Then continue to next character (we don't type the correct char).
                    # This ensures the configured percentage of mistakes appear in final output.
                    continue
                else:
                    # Otherwise simulate backspace to remove the wrong char (human-like)
                    pyautogui.press('backspace')
                    time.sleep(random.uniform(error_correction[0] / 2, error_correction[1] / 2))

                    # Now type the correct character (and continue; avoid double-typing)
                    pyautogui.write(char)
                    time.sleep(random.uniform(*char_delay))
                    # We've already handled this char (corrected) so skip the normal write below
                    continue

            # --- Normal clean typing (no visible mistake triggered) ---
            pyautogui.write(char)
            time.sleep(random.uniform(*char_delay))
...
class TimingControls:
    def __init__(self, parent):
        ...
        # Character Timing (UPDATED defaults to requested range)
        tk.Label(self.frame, text="Char Delay:").grid(row=0, column=0, sticky="e")
        self.char_min = tk.Entry(self.frame, width=8)
        ...
        self.char_max = tk.Entry(self.frame, width=8)
...
# In dev mode: resolves to <project_root>/assets/icons/shukylogo.ico
# In EXE mode: resolves to bundled shukylogo.ico in PyInstaller temp dir
try:
    _icon_path = resource_path(os.path.join("assets", "icons", "shukylogo.ico"))
...
app_title = tk.Label(title_frame, text="Shuky Typing Assistant v3.0.1", font=title_font, fg=primary_color, bg=bg_color)
